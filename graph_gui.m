function g=graph_gui(A,xy,drawDef,delDef,vertexChked,edgeChked)
...
%
% Edited for use with SBEToolbox.
% (C) is for SBEToolbox, not for graph_gui.
%
% Systems Biology and Evolution Toolbox (SBEToolbox).
% Authors: James Cai, Kranti Konganti.
% (C) Texas A&M University.
%
% $LastChangedDate: 2013-02-14 13:31:31 -0600 (Thu, 14 Feb 2013) $
...
% Initially graph is not drawn
setpref('SBEToolbox', 'graph_gui', 0);
if ~ispref('SBEToolbox', 'graph_gui_drawn')
    addpref('SBEToolbox', 'graph_gui_drawn', 0);
else
    setpref('SBEToolbox', 'graph_gui_drawn', 0);
end

% set up the figure window
g=figure('Name','Network Editor', ...
    'Numbertitle','off', ...
    'Menubar', 'none', ...
    'Toolbar', 'figure', ...
...
%----- callback functions ------------------------
    ...
    function dgplot(A,xy)
        n = size(A,1);
        A = max(0,min(1,ceil(abs(double(A)))));
        uA = A.*A'.*(1-eye(n));
        ...
        hold on
        plot(ux,uy,'k-','LineSmoothing','On','color',[.6 .6 .6])
        plot(dxu,dyu,'r:','LineSmoothing','On','color',[.6 .6 .6])
        plot(dxl,dyl,'b--','LineSmoothing','On','color',[.6 .6 .6])
        plot(ix,iy,'ko','LineSmoothing','On','color',[.6 .6 .6])
        graph=plot(xy(:,1),xy(:,2),'k.');
         set(graph, 'MarkerEdgeColor','r',...
             'MarkerFaceColor', 'r', 'MarkerSize', 15);
         
        if n > size(sbeNode, 1)
            for newN = size(sbeNode, 1) + 1:n
                sbeNode{newN} = num2str(newN);
            end
        end
    ...
    function saveGraph(varargin)        
        ...
        %end
        sbeG=logical(A);
        if (size(sbeG, 1) ~= in_G_N && size(sbeG, 2) ~= in_G_E)
            setpref('SBEToolbox', 'graph_gui_drawn', 1);
        end
        %sbeNode=num2cellstr(1:size(sbeG,1));
        %sbeNode=cellfun(@num2str,num2cell(1:size(sbeG,1)),'Uniform',false);
        if getpref('SBEToolbox', 'graph_gui_drawn') == 1
            setpref('SBEToolbox', 'graph_gui', 1);
        end
        close
    end
